/**
 * Test Freya's Memory — collapsible chat panel with mock reply that references store memory with [📎 From Memory] tags.
 */
import { useState } from 'react';
import { C, F, R, S } from '../../tokens';

function buildMockReply(freyaMemory) {
  const parts = [];
  if (freyaMemory?.brand?.length) {
    const first = freyaMemory.brand[0]?.content || '';
    if (first) parts.push(`Your brand tone is confident and data-driven [📎 From Memory: Brand Memory].`);
    const prohibited = freyaMemory.brand.find((e) => e.content?.includes('Prohibited'));
    if (prohibited) parts.push(`I avoid words like 'revolutionary' or 'game-changing' per your guidelines [📎 From Memory: Brand Memory].`);
  }
  if (freyaMemory?.audience?.length) {
...
export default function ARIAMemoryChat({ freyaMemory }) {
  ...
  const handleSend = () => {
    ...
    if (!trimmed) return;
    setReply(buildMockReply(freyaMemory));
    setInput('');
  };
      ...
      {!collapsed && (
        <div style={{ padding: `0 ${S[4]} ${S[4]}`, borderTop: `1px solid ${C.border}` }}>
          <p style={{ fontFamily: F.body, fontSize: '12px', color: C.textMuted, margin: `0 0 ${S[3]} 0` }}>
            Ask Freya something about your brand — it should recall your memory entries.
          </p>
