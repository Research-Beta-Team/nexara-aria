...
import { KNOWLEDGE_CATEGORIES, KNOWLEDGE_HEALTH, getMissingCategories } from '../../../data/freyaKnowledge';

export default function KnowledgeHealthBar({ onAddMissing }) {
  const missing = getMissingCategories();
  const missingText = missing.length > 0
    ? `Missing: ${missing.join(' and ')}. Add these to improve Freya's recommendations by ~35%.`
    : "Freya's knowledge is in good shape.";
  ...
  return (
    <div style={{
      ...
      <div style={{ fontFamily: F.body, fontSize: '14px', fontWeight: 600, color: C.textPrimary, marginBottom: S[2] }}>
        Freya Knowledge Completeness: {KNOWLEDGE_HEALTH.overallScore}%
      </div>
      <div style={{
        height: '10px',
        borderRadius: R.pill,
