/**
 * Freya Engine — core agentic brain.
 * Manages conversation loop, tool use, streaming, and demo mode.
 */

import { FREYA_TOOLS } from './FreyaTools.js';
import { execute } from './FreyaToolExecutor.js';
import { freyaMemory } from './FreyaMemory.js';
import { buildSystemPrompt } from './FreyaSystemPrompt.js';
...
export class FreyaEngine {
  ...
  async _runDemoMode(userMessage) {
    ...
    const finalText = script?.finalText || `Request received: "${userMessage.slice(0, 50)}...". Configure VITE_ANTHROPIC_API_KEY for full Freya.`;
