...
export default function SocialMedia() {
  ...
  const handleGenerateSocialPosts = async () => {
    ...
    const res = await copywriter.activate('Generate social posts', {
  ...
  const handleSchedulePosts = async () => {
    ...
    toast.success('Outreach Agent activated...');
    const res = await outreach.activate('Schedule social posts', {
      platforms: ['linkedin', 'twitter', 'instagram', 'facebook'],
  ...
  return (
    <div style={{ padding: S[6], display: 'flex', flexDirection: 'column', gap: S[5], minHeight: '100%' }}>
      ...
      <div style={cardStyle}>
        ...
        <p style={{ fontFamily: F.body, fontSize: '13px', color: C.textSecondary, marginBottom: S[4] }}>
          Use AI agents to generate platform-optimized posts and schedule them across channels.
        </p>
        <div style={{ display: 'flex', gap: S[3], flexWrap: 'wrap', marginBottom: S[4] }}>
          ...
          <button
            ...
            style={{
              ...btn.secondary,
              fontSize: '13px',
              opacity: outreach.isActive ? 0.7 : 1,
              cursor: outreach.isActive ? 'wait' : 'pointer',
            }}
          >
            Schedule posts
          </button>
        </div>
        <div style={{ fontFamily: F.mono, fontSize: '10px', color: C.textMuted, marginBottom: S[3] }}>
          Generated by Content Agent
        </div>
        {copywriter.isActive && (
          <div style={{ marginBottom: S[3] }}>
            <AgentThinking agentId="copywriter" task="Generating platform-optimized posts for LinkedIn, Twitter, Instagram, Facebook..." />
          </div>
        )}
        {outreach.isActive && (
          <div style={{ marginBottom: S[3] }}>
            <AgentThinking agentId="outreach" task="Scheduling posts across social channels..." />
          </div>
        )}
        {generateResult && (
          <div style={{ marginBottom: S[3] }}>
            <AgentResultPanel result={generateResult} />
          </div>
        )}
        {scheduleResult && (
          <div style={{ marginBottom: S[3] }}>
            <AgentResultPanel result={scheduleResult} />
          </div>
        )}
      </div>
