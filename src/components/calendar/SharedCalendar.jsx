...
import { TYPE_COLORS } from '../../data/calendar';
          ...
          return (
            <div
              ...
              style={{
                ...
                borderRight,
                borderBottom,
                backgroundColor: !day ? C.surface3 : isToday ? 'rgba(61,220,132,0.05)' : 'transparent',
...
/**
 * Shared calendar: month/week/day views, type filter, event detail panel.
 * @param {Object} props
 * @param {Array} props.events - Calendar events (each: id, date, time, type, title, description, channel, status; optional campaignId)
 * @param {React.ReactNode} [props.extraFilterSlot] - Optional slot in toolbar (e.g. campaign filter)
 */
export default function SharedCalendar({ events = [], extraFilterSlot }) {
  const toast = useToast();
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ padding: S[5], display: 'flex', flexDirection: 'column', gap: S[4] }}>
        ...
        {view === 'month' && (
          <MonthView events={filteredEvents} currentDate={currentDate} onSelect={setSelectedEvent} />
        )}
        {view === 'week' && (
          <WeekView events={filteredEvents} currentDate={currentDate} onSelect={setSelectedEvent} />
        )}
        {view === 'day' && (
          <DayView events={filteredEvents} currentDate={currentDate} onSelect={setSelectedEvent} />
        )}
