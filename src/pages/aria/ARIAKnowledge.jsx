...
import {
  ...
  getCategoryLabel,
} from '../../data/ariaKnowledge';
import KnowledgeHealthBar from '../../components/aria/knowledge/KnowledgeHealthBar';
import KnowledgeDocCard from '../../components/aria/knowledge/KnowledgeDocCard';
import KnowledgeFactCard from '../../components/aria/knowledge/KnowledgeFactCard';
import ARIABeliefEditor from '../../components/aria/knowledge/ARIABeliefEditor';
import AddKnowledgeModal from '../../components/aria/knowledge/AddKnowledgeModal';

const initialBeliefs = () => ({
  brand: [...ARIA_BELIEFS.brand],
  icp: [...ARIA_BELIEFS.icp],
  communication: [...ARIA_BELIEFS.communication],
  campaign: [...ARIA_BELIEFS.campaign],
  competitor: [...ARIA_BELIEFS.competitor],
  product: [...ARIA_BELIEFS.product],
});
...
export default function ARIAKnowledge() {
  ...
  const [expanded, setExpanded] = useState(() => {
    const o = {};
    KNOWLEDGE_CATEGORIES.forEach((c) => { o[c.id] = (ARIA_BELIEFS[c.id]?.length ?? 0) > 0; });
    return o;
  });
